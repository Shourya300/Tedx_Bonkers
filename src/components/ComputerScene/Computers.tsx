    ...
    return (
        <group {...props} dispose={null}>
            ...
            {SCREENS.map((screen: any, index: number) => {
                const displayText = (sponsors.length > 0 ? sponsors[0].name : "TEDx") || "TEDx";
                return (
                    <ScreenLogo
                        key={index}
                        text={displayText}
        ...
        <group {...props}>
            ...
            <mesh geometry={nodes[panel].geometry}>
                <meshBasicMaterial toneMapped={false}>
                    <RenderTexture width={200} height={200} attach="map" anisotropy={1}>
                        {children}
                    </RenderTexture>
                </meshBasicMaterial>
            </mesh>
...
function TextPlane({ text, invert }: { text: string, invert: boolean }) {
    ...
    // Random tech-related phrases to display after the sponsor name
    const randomPhrases = useMemo(() => [
        "Innovation", "Technology", "Future", "Digital", "Smart Solutions",
        "Next Gen", "AI Powered", "Cloud First", "Data Driven", "Breakthrough",
        "Revolutionary", "Cutting Edge", "Transform", "Innovate", "Disrupt",
        "Scale Up", "Growth", "Vision", "Excellence", "Leadership"
    ], [])
    
    const randomText = useMemo(() => {
        const phrases: string[] = []
        for (let i = 0; i < 5; i++) {
            phrases.push(randomPhrases[Math.floor(Math.random() * randomPhrases.length)])
        }
        return phrases.join(' • ')
    }, [randomPhrases])
    
    const fullText = `${text} • ${randomText} • ${text} • ${randomText}`


    useFrame((state, delta) => {
  globalScroll += delta * 1.5 // speed
})
    ...
    return (
        <Text
  ref={textRef}
  position={[-globalScroll % 10, 0, 0]}
  fontSize={1.1}
  maxWidth={8}
  lineHeight={1}
  letterSpacing={-0.04}
  textAlign="center"
  color={invert ? '#E62B1E' : 'white'}
  anchorX="center"
  anchorY="middle"
>
  {fullText}
